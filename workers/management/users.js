...
export async function getUserProfile(db, userId) {
  const user = await db.prepare(
    `SELECT id, email, name, plan_type, plan_started_at, plan_expires_at, created_at FROM users WHERE id = ?`
  ).bind(userId).first();
...
export async function updateUserPlan(db, userId, newPlanType, expiresAt = null) {
  ...
  await db.prepare(
    `UPDATE users SET plan_type = ?, plan_started_at = ?, plan_expires_at = ?, updated_at = ? WHERE id = ?`
  ).bind(newPlanType, now, expiresAt, now, userId).run();
...
export async function getUserActivity(db, userId, limit = 20) {
  ...
  const uploads = await db.prepare(
    `SELECT 'csv_upload' as activity_type, filename as description, uploaded_at as timestamp, row_count as metadata FROM csv_uploads WHERE user_id = ? ORDER BY uploaded_at DESC LIMIT ?`
  ).bind(userId, limit).all();

  // Get recent client creations (if enterprise user)
  const clients = await db.prepare(
    `SELECT 'client_created' as activity_type, company_name as description, created_at as timestamp, NULL as metadata FROM clients WHERE user_id = ? ORDER BY created_at DESC LIMIT ?`
  ).bind(userId, limit).all();
