...
- @returns {Promise<Object>} Alert statistics
  */
  ...
  const query = clientId
  ? `SELECT  COUNT(*) as total, SUM(CASE WHEN read_at IS NULL THEN 1 ELSE 0 END) as unread, SUM(CASE WHEN severity = 'critical' AND dismissed_at IS NULL THEN 1 ELSE 0 END) as critical, SUM(CASE WHEN severity = 'warning' AND dismissed_at IS NULL THEN 1 ELSE 0 END) as warnings FROM alerts WHERE user_id = ? AND client_id = ?`
  : `SELECT  COUNT(*) as total, SUM(CASE WHEN read_at IS NULL THEN 1 ELSE 0 END) as unread, SUM(CASE WHEN severity = 'critical' AND dismissed_at IS NULL THEN 1 ELSE 0 END) as critical, SUM(CASE WHEN severity = 'warning' AND dismissed_at IS NULL THEN 1 ELSE 0 END) as warnings FROM alerts WHERE user_id = ?`;
