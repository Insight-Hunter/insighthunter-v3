export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const routes = {
        'POST:/insights': handleGenerateInsights,
        'POST:/forecast': handleGenerateForecast,
        'POST:/search': handleSemanticSearch,
        'POST:/categorize': handleAICategorize,
        'GET:/dashboard': handleDashboardData,
      };
...
async function handleGenerateInsights(request, env, user, corsHeaders) {
  ...
  const { results: transactions } = await env.DB
    .prepare('SELECT * FROM transactions WHERE client_id = ? ORDER BY date DESC')
    .bind(clientId)
    .all();
...
/**
 * Generate financial forecast using Llama 3.1
 */
async function handleGenerateForecast(request, env, user, corsHeaders) {
  ...
  const { results: transactions } = await env.DB
    .prepare('SELECT * FROM transactions WHERE client_id = ? AND date >= ? ORDER BY date ASC')
    .bind(clientId, twelveMonthsAgo)
    .all();
...
async function handleSemanticSearch(request, env, user, corsHeaders) {
  ...
  if (transactionIds.length === 0) {
    return jsonResponse({ query, results: [] }, 200, corsHeaders);
  }
...
async function handleAICategorize(request, env, user, corsHeaders) {
  ...
  return jsonResponse({ category, confidence, description, amount }, 200, corsHeaders);
}
...
async function handleDashboardData(request, env, user, corsHeaders) {
  ...
  const { results: transactions } = await env.DB
    .prepare('SELECT * FROM transactions WHERE client_id = ? AND date >= ? ORDER BY date DESC')
    .bind(clientId, thirtyDaysAgo)
    .all();
...
const response = await env.AI.run('@cf/meta/llama-3.1-8b-instruct', {
    messages: [
      { role: 'system', content: 'You are a CFO providing financial insights. Respond ONLY with valid JSON, no additional text.' },
...
"metadata": {"trend": "string", "notes": "string"}
  ...
  const response = await env.AI.run('@cf/meta/llama-3.1-8b-instruct', {
    messages: [
      { role: 'system', content: 'You are a financial forecasting expert. Respond ONLY with valid JSON.' },
      { role: 'user', content: prompt }
    ],
    max_tokens: 1024
  });
...
function calculateMetrics(transactions) {
  ...
  return { totalIncome, totalExpenses, netIncome, profitMargin, expensesByCategory };
}
...
async function authenticateRequest(request, env) {
  ...
  const session = await env.KV.get(`session:${sessionId}`, 'json');t
