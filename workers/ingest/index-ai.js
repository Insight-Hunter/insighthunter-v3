...
if (!hasAmount) {
errors.push(‘Missing amount column’);
}
...
- Process CSV with AI categorization and Vectorize
  */
...
if (!upload) {
return jsonResponse({ error: ‘Upload not found’ }, 404, corsHeaders);
}
...
// Batch insert transactions into D1
const placeholders = transactions.map(() => ‘(?, ?, ?, ?, ?, ?, ?, ?, unixepoch())’).join(’,’);
const values = transactions.flatMap(t => [
t.id, t.client_id, t.date, t.description, t.amount, t.type, t.category, t.confidence
]);
...
).bind(…values.map(v => v)).run();

// Batch insert embeddings into Vectorize
...
- Process transaction with AI categorization
  */
...
- AI categorization using Llama 3.1
  */
  ...
  {
  role: ‘system’,
  content: ‘You are a financial categorization expert. Categorize into: Payroll, Rent, Utilities, Marketing, Supplies, Insurance, Travel, Meals, Software, Professional Services, Sales, Services, Interest, Other Income, Other Expenses. Respond with ONLY the category name.’
  },
  {
  role: ‘user’,
