...
export default {
  async fetch(request: Request, env: Env) {
    ...
    if (request.method === 'POST' && url.pathname === '/api/auth/signup') {
      ...
      await env.USERS_DB.prepare('INSERT INTO users (email, password_hash) VALUES (?, ?)').bind(email, hash).run();
    ...
    if (request.method === 'POST' && url.pathname === '/api/auth/signin') {
      ...
      const user = await env.USERS_DB.prepare('SELECT password_hash FROM users WHERE email = ?').bind(email).first();
    ...
    if (request.method === 'POST' && url.pathname === '/api/business') {
      ...
      await env.USERS_DB.prepare('UPDATE users SET business_name = ?, industry = ? WHERE email = ?').bind(businessName, industry, email).run();
    ...
    if (request.method === 'GET' && url.pathname === '/api/business') {
      ...
      const user = await env.USERS_DB.prepare('SELECT business_name, industry FROM users WHERE email = ?').bind(email).first();
