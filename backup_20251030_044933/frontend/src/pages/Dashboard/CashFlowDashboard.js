...
` ``;
    ...
    const loadCashFlowData = () => __awaiter(this, void 0, void 0, function* () {
        setLoading(true);
        `` `
...
<button onClick={loadCashFlowData} disabled={loading} className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
...
```
      ...
      <div className="mt-4 flex items-center gap-4 bg-blue-50 border border-blue-200 rounded-lg p-3">
        ...
        <select value={refreshInterval} onChange={(e) => setRefreshInterval(Number(e.target.value))} disabled={!autoRefresh} className="text-sm px-3 py-1 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500">
  ...
  <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
        ...
        <div className={`flex items-center text-sm ${cashFlowData.projectedCash['30days'] < cashFlowData.currentCash
        ? 'text-red-600'
        : 'text-green-600'}`}>
    ...
    {/* Alerts */}
    {cashFlowData.alerts.length > 0 && (<div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
          <AlertTriangle className="w-5 h-5 text-yellow-600"/>
          Active Alerts
        </h3>
        <div className="space-y-3">
          {cashFlowData.alerts.map((alert) => (<div key={alert.id} className={`p-4 rounded-lg border ${alert.type === 'warning' ? 'bg-yellow-50 border-yellow-200' :
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <button className={`text-sm font-medium ${alert.type === 'warning' ? 'text-yellow-700' :
                alert.type === 'success' ? 'text-green-700' :
                    'text-blue-700'}`}>
                    {alert.action} →
                  </button>
                </div>
                <span className={`text-xs px-2 py-1 rounded-full font-medium ${alert.severity === 'high' ? 'bg-red-100 text-red-700' :
    ...
    <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
      ...
      <ResponsiveContainer width="100%" height={400}>
        <AreaChart data={combinedTrend}>
          ...
          <XAxis dataKey="date" stroke="#6b7280" tickFormatter={(date) => new Date(date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}/>
          <YAxis stroke="#6b7280" tickFormatter={(value) => `$${(value / 1000).toFixed(0)}k`}/>
          <Tooltip contentStyle={{ backgroundColor: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px' }} formatter={(value, name) => {
        if (!value)
            return null;
        return [`$${value.toLocaleString()}`, name];
    }} labelFormatter={(date) => new Date(date).toLocaleDateString()}/>
          <Legend />
          <Area type="monotone" dataKey="upper" stroke="none" fill="url(#colorConfidence)" name="Confidence Range"/>
          <Area type="monotone" dataKey="lower" stroke="none" fill="#fff"/>
          <Area type="monotone" dataKey="actual" stroke="#3b82f6" strokeWidth={3} fill="url(#colorActual)" name="Actual Balance"/>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
        ...
        <div className="space-y-4">
          ...
          {cashFlowData.inflows.overdue > 0 && (<div className="flex items-center justify-between p-4 bg-red-50 rounded-lg border border-red-200">
              <div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {cashFlowData.upcomingTransactions.map((txn) => (<tr key={txn.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${txn.status === 'recurring' ? 'bg-blue-100 text-blue-800' :
            txn.status === 'scheduled' ? 'bg-purple-100 text-purple-800' :
                ...
                <td className={`px-4 py-3 whitespace-nowrap text-right text-sm font-semibold ${txn.type === 'inflow' ? 'text-green-600' : 'text-red-600'}`}>
                  {txn.type === 'inflow' ? '+' : ''}${Math.abs(txn.amount).toLocaleString()}
                </td>
                <td className="px-4 py-3 whitespace-nowrap text-center">
                  <div className="flex items-center justify-center gap-2">
                    <div className="w-16 bg-gray-200 rounded-full h-2">
                      <div className="bg-blue-600 h-2 rounded-full" style={{ width: `${txn.confidence * 100}%` }}></div>
                    </div>
