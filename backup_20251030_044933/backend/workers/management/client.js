    ...
    if (url.pathname === '/clients' && request.method === 'POST') {
      try {
        ...
        const result = await env.DB.prepare(
          `INSERT INTO clients (user_id, company_name, contact_email, contact_phone, status, created_at) 
           VALUES (?, ?, ?, ?, ?, ?)`
        )
          .bind(userId, company_name, contact_email, contact_phone, 'active', new Date().toISOString())
          .run();
